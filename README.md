...
菜单项**水平平铺**展示，使用**方向键 `←`/`→` 左右移动、`↑`/`↓` 上下换行**，回车确认（ESC 取消）；非终端环境（管道/脚本）自动回退为序号输入。列数按终端宽度自适应（越宽的窗口每行放得越多），选中某项执行后自动返回菜单，选择「退出」结束。

> Linux 下菜单多一项「查看任务」（在「证书更新任务」之后）。

**分区常驻界面**：交互菜单采用常驻分区布局——上方主区域显示证书列表，中间为操作菜单（辅区域），底部反馈区显示操作输出。选择菜单、执行操作时**只刷新变化的区域**，不会重新渲染整个界面：

```
  ══════ SSL Assistant 操作菜单 ══════
   ID  域名            状态      到期时间      ← 主区域：证书列表
    1  example.com     有效      2026-12-31
    2  www.example.com 有效      2026-12-31

   1. 初始化程序  2. 添加证书  3. 删除证书    ← 辅区域：菜单（仅高亮刷新）
   4. 查看证书    5. 更新证书  6. 快速添加域名
   ...
  ══════ 更新证书 ══════                     ← 反馈区：操作输出
   正在更新 example.com... 更新完成
```
