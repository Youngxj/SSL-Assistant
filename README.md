...
> 自动检索到证书配置后不会再询问自定义路径；仅当默认路径（宝塔 `/www/server/panel/vhost/nginx/*.conf`、`/www/server/apache/vhost/*.conf`、1Panel `/opt/1panel/www/conf.d/*.conf`、`/etc/nginx`、`/etc/apache2` 等）未找到证书时，才提示可手动补充（直接回车跳过）。
