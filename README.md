...
- 自动化管理：自动寻找 Nginx / Apache 配置文件（已兼容宝塔面板、1Panel），获取域名和证书信息 🧐
...
```
========== SSL Assistant 操作菜单 ==========
  1. 初始化程序    2. 添加证书      3. 删除证书
  4. 更新证书      5. 快速添加域名  6. 证书更新任务
  7. 修改密钥      8. 修改重载命令  9. 修改提前更新天数
 10. 查看配置信息  11. 显示版本信息 12. 检查更新
 13. 退出
============================================
```

菜单项**水平平铺**展示，使用**方向键 `←`/`→` 左右移动、`↑`/`↓` 上下换行**，回车确认（ESC 退出）；非终端环境（管道/脚本）自动回退为序号输入。列数按终端宽度自适应（越宽的窗口每行放得越多），选中某项执行后自动返回菜单，选择「退出」结束。

> Linux 下菜单多一项「查看任务」（在「证书更新任务」之后）。

**全 TUI 实时界面**：交互菜单基于成熟 TUI 框架 [tview](https://github.com/rivo/tview) 构建，所有操作都在 TUI 内完成、**不退出界面、不重复加载**：
- **证书列表**常驻主区（结构化表格，状态彩色标注，操作后自动刷新）——即实时查看，无需单独"查看证书"菜单
- 选择菜单项后，操作**输出实时渲染在下方反馈区**（更新进度、配置内容、检查更新结果等）
- 需要输入时（添加域名、修改密钥、勾选站点等）弹出**模态输入框/确认框/多选列表**，输入完成自动回到界面
- 框架自动处理方向键导航、窗口 resize、终端重绘，跨 cmd / PowerShell / Windows Terminal / Linux 终端行为一致：

```
┌────────────────── 证书列表 ──────────────────┐
│ ID  域名             状态      剩余天数       │
│  1  example.com      有效       30天         │
│  2  www.example.com  有效       15天         │
└──────────────────────────────────────────────┘
┌────────────────── 操作输出 ──────────────────┐
│ ══════ 检查更新 ══════                        │
│ 最新版本: v1.2.3                               │
