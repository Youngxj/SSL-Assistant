...
========== SSL Assistant 操作菜单 ==========
  ...
  4. 查看证书      5. 更新证书      6. 快速添加域名
  7. 证书更新任务  8. 修改密钥      9. 修改重载命令
 10. 修改提前更新天数             11. 查看配置信息
 12. 显示版本信息  13. 检查更新    14. 退出
============================================
```
...
**稳定 TUI 界面**：交互菜单基于成熟 TUI 框架 [tview](https://github.com/rivo/tview) 构建——上方主区域显示证书列表（支持滚动），中间为操作菜单（平铺、方向键导航），底部为操作提示。选择操作后**以普通终端模式执行**（输出与输入提示原生可用），执行完自动回到 TUI 并刷新列表。框架自动处理方向键导航、窗口 resize、终端重绘，跨 cmd / PowerShell / Windows Terminal / Linux 终端行为一致：

```
┌─────────────── 证书列表 ───────────────┐
│ ID  域名            状态      到期时间  │
│  1  example.com     有效      2026-12-31│
│  2  www.example.com 有效      2026-12-31│
└────────────────────────────────────────┘
  1. 初始化程序  2. 添加证书  3. 删除证书
  4. 查看证书    5. 更新证书  6. 快速添加域名
  ...
