...
初始化程序，设置证书信息获取的凭证和证书更新后需要执行的命令。初始化完成后，程序会自动寻找宝塔/1Panel/原生 Nginx 的配置文件，**列出所有检索到的域名**，使用回车勾选（输入序号切换勾选，空格分隔可一次切换多个，直接回车确认）要添加的域名，确认后自动将勾选的域名添加到服务中。

```
[ ] 1. example.com
[x] 2. www.example.com
输入序号切换勾选（多个用空格分隔，直接回车确认）: 1
```
...
快速添加域名，程序会自动寻找 Nginx 配置文件，**列出所有检索到的域名**，使用回车勾选要添加的域名，确认后自动将勾选的域名添加到服务中，以便后面的更新操作。
