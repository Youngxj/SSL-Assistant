...
初始化程序，设置证书信息获取的凭证和证书更新后需要执行的命令。初始化完成后，程序会自动寻找宝塔/1Panel/原生 Nginx 的配置文件，**列出所有检索到的域名**，使用**方向键 ↑/↓ 移动高亮、空格勾选、回车确认**（ESC 取消）要添加的域名，确认后自动将勾选的域名添加到服务中。非交互环境（管道/脚本）下自动回退为序号输入模式。
